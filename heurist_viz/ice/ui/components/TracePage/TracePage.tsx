...
const applyUpdates = (calls: Calls, updates: Record<string, unknown>) => {
  ...
  console.log("Updated calls:", calls);
};
...
const TreeProvider = ({ traceId, children }: { traceId: string; children: ReactNode }) => {
  ...
  console.log("TreeProvider render:", {
    traceId,
    rootId,
    callsLength: Object.keys(calls).length,
    selectedId,
    autoselected
  });
  ...
  useEffect(() => {
    if (!autoselected) {
      console.log("Attempting autoselection with calls:", calls);
      const firstRoot = Object.keys(calls[traceId]?.children ?? {})[0];
      console.log("Found firstRoot:", firstRoot);
      if (firstRoot) {
        setRootId(firstRoot);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        // Simple fetch of entire file
        const response = await fetch(`${urlPrefix(traceId)}/trace.jsonl`);
        const text = await response.text();
        
        // Split into lines and filter empty ones
        const lines = text.split('\n').filter(line => line.trim());
        
        // Only update if we have more lines than current offset
        if (lines.length > traceOffsetRef.current) {
          console.log(`Found new lines: ${lines.length - traceOffsetRef.current}`);
          
          setCalls(calls =>
            produce(calls, draft => {
              // Process only new lines
              lines.slice(traceOffsetRef.current).forEach(line => {
                try {
          ...
          traceOffsetRef.current = lines.length;
        }
      } catch (e) {
  ...
  function getBlockValue<T>(blockAddress: BlockAddress<T>): T | undefined {
    ...
    if (!block) {
      // Only fetch if we haven't requested this block yet
      if (!blockRequests.current.has(blockNumber)) {
        blockRequests.current.add(blockNumber);
        const pollBlock = async () => {
          try {
            const response = await fetch(`${urlPrefix(traceId)}/block_${blockNumber}.jsonl`);
            const text = await response.text();
            const lines = text.split("\n").filter(line => line.trim());
            ...
            // If we don't see an "end" marker, poll again after a delay
            if (!lines.includes("end")) {
              setTimeout(pollBlock, 1000);
            }
          } catch (error) {
    ...
    // If we have the block, try to parse the line
    try {
      const line = block[blockLineno];
      return line ? JSON.parse(line) : undefined;
    } catch (error) {
      console.error("Error parsing block line:", error);
      return undefined;
    }
  }
...
const InputOutputContent = ({ args, records, result }: InputOutputContentProps) => {
  ...
  const argsValue = args ? getBlockValue(args) : undefined;
  const resultValue = result ? getBlockValue(result) : undefined;
  
  console.log("InputOutput values:", {
    rawArgs: args,
  ...
  return (
    <>
      <Json name="Inputs" value={argsValue} />
      {!isEmpty(records) && (
        <Json
          name="Records"
          value={Object.fromEntries(
            Object.entries(records || {}).map(([key, value]) => [
              key,
              value ? getBlockValue(value) : undefined
            ])
          )}
        />
      )}

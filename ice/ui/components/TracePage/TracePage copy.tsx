...
const TreeProvider = ({ traceId, children }: { traceId: string; children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      console.log("Starting poll for trace data");
      let delay = 1_000;
      try {
        const url = `${urlPrefix(traceId)}/trace.jsonl`;
        const offset = traceOffsetRef.current;
        const contentLength = await getContentLength(url);
        console.log("Poll contentLength:", contentLength, "offset:", offset);
        
        if (offset >= contentLength) {
          console.log("No new trace data available");
          return;
        }
        ...
        console.log("Poll received text length:", text.length);
        ...
        setCalls(calls =>
          produce(calls, draft => {
            ...
            updates.forEach(update => applyUpdates(draft, update));
          }),
        );
      } catch (e) {
